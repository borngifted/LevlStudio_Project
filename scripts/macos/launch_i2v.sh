#!/bin/bash
# WAN I2V (Image-to-Video) GGUF + LoRA Workflow Launcher for macOS
# Launches ComfyUI and executes the I2V workflow with interactive input
...
read -p "Enter path to input image (or press Enter for example): " input_image
input_image=${input_image:-"examples/input_image.jpg"}

read -p "Enter output directory (or press Enter for default): " output_dir
output_dir=${output_dir:-"ComfyUI/output/i2v"}
...
export LEVL_INPUT_DIR="$input_image"
export LEVL_REF_IMAGE=""
export LEVL_OUTPUT_DIR="$output_dir"

echo "Launching I2V GGUF + LoRA workflow..."
if python3 tools/levl_enqueue.py --workflow "ComfyUI/workflow_results/goshniiAI-WAN 2.2 Image-to-Video l GGUF + LoRA.json" --host 127.0.0.1 --port 8188; then
    echo ""
    echo "================================"
