...
# Optional LLM libs (only imported when needed)
def _have_openai():
    try:
...
def _blender_cmd(blender_path: str,
                 runner_py: str,
                 addon_py: str,
                 assets_json: str,
                 scenes_json: str,
                 project_root: str,
                 operator: str,
                 operator_args: Dict[str, Any]) -> List[str]:
    op_args = ["--op", operator,
               "--assets", assets_json,
               "--scenes", scenes_json,
...
def _run_blender(args, operator, operator_args) -> Dict[str, Any]:
    ...
    try:
        ...
        # keep raw stdout/stderr for debugging
        pass
    return out
...
def _llm_openai(prompt: str, model: Optional[str], temperature: float) -> str:
    ...
    try:
        ...
        resp = client.chat.completions.create(
            ...
            messages=[
                {"role":"system","content":"You are a Blender pipeline and add-on engineering assistant. Return concise, actionable fixes."},
...
def _troubleshoot_prompt(scene_id: str,
                         assets_json: str,
                         scenes_json: str,
                         resolve: Dict[str, Any],
                         stdout: str,
                         stderr: str,
                         max_log_chars: int = 4000) -> str:
...
RESOLVE REPORT (Python dict/JSON-ish):
{json.dumps(resolve, indent=2)}

CURRENT assets.json (abridged if large):
{json.dumps(assets, indent=2) if assets else "N/A"}

CURRENT scenes.json (abridged if large):
...
TASKS:
1) Identify the most likely causes for any 'procedural' or missing assets.
...
4) Keep suggestions short, numbered, and directly actionable.

OUTPUT FORMAT (strict):
- "suggestions": short bullet list
- "proposed_assets_json": JSON or empty
- "proposed_scenes_json": JSON or empty
"""

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--blender", required=True, help="Path to Blender executable")
    ap.add_argument("--addon", required=True, help="Path to LevlStudio add-on .py")
    ap.add_argument("--assets", required=True, help="Path to assets.json")
    ap.add_argument("--scenes", required=True, help="Path to scenes.json")
    ap.add_argument("--project", required=True, help="Project root for // paths")
    ap.add_argument("--runner", default=os.path.join(os.path.dirname(__file__), "blender_operator_runner.py"), help="Runner script that executes inside Blender")
    ...
    def build_scene(scene_id: str,
        ...
        return _run_blender(args, "levl.build_scene_and_optional_export", operator_args)
    ...
    @server.tool()
    def troubleshoot_scene(scene_id: str,
                           provider: str = "openai",
                           model: str = "",
                           temperature: float = 0.2) -> dict:
        """
        Runs resolve_report, then asks an LLM for concrete fixes.
        provider: 'openai' or 'gemini'
        model: optional model name (e.g., gpt-4o-mini, gemini-1.5-flash)
        """
        # Step 1: Resolve report
        ...
        suggestions = text
        proposed_assets = None
        proposed_scenes = None
        try:
            # naive parse: look for JSON blocks
            import re
            blocks = re.findall(r'\{(?:[^{}]|(?R))*\}', text, flags=re.DOTALL)
            # take the largest block as potential JSON
            if blocks:
                blocks_sorted = sorted(blocks, key=len, reverse=True)
                for blk in blocks_sorted:
                    try:
                        obj = json.loads(blk)
                        if "proposed_assets_json" in obj or "proposed_scenes_json" in obj or "suggestions" in obj:
